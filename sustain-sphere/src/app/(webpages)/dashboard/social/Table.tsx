

const Table = () => {
 
  const tableData = [
    {
      company: 'Nvidia Corporation',
      logoUrl: 'https://www.nvidia.com/content/dam/en-zz/Solutions/about-nvidia/logo-and-brand/02-nvidia-logo-color-grn-500x200-4c25-p@2x.png',
      col1: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
      col2: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
      col3: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
      col4: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
      col5: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
      col6: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>'
    },
    {
      company: 'Adobe Inc.',
      logoUrl: 'data:image/png;base64,',
      col1: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
      col2: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
      col3: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
      col4: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
      col5: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
      col6: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>'
    },
    {
        company: 'Advanced Micro Devices Inc.',
        logoUrl: 'https://1000logos.net/wp-content/uploads/2020/05/AMD-Logo.png',
        col1: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col2: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col3: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col4: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col5: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col6: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>'
      },
      {
        company: 'Broadcom Inc.',
        logoUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQdJQ7YLk_nSHH-6W7i6dcngSNu6e6ndqRrsKsJZA05&s',
        col1: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col2: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col3: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col4: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col5: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col6: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>'
      },
      {
        company: 'Cisco Systems Inc.',
        logoUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT0K6FNDVczOs64YPG2mvH1YmjvvwnRkyLjih9ymqNU&s',
        col1: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col2: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col3: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col4: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col5: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>',
        col6: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="green" class="bi bi-check2" viewBox="0 0 16 16"><path d="M1.293 7.293a1 1 0 0 1 1.414-1.414L6 9.586l7.293-7.293a1 1 0 1 1 1.414 1.414l-8 8a1 1 0 0 1-1.414 0l-8-8z"/></svg>'
      },
    // Add more rows as needed
  ];

  return (
    <div className="">
      <table className="min-w-full border-collapse border border-gray-300 text-black">
        <thead>
          <tr className="h-16 text-center">
            <th className="px-6 py-3 bg-gray-100  text-xs leading-4 font-bold  uppercase ">
           
            </th>
            <th className="px-16 py-3 w-10 bg-gray-100  text-xs leading-4 font-bold  uppercase ">
              Carbon Related (5)
            </th>
            <th className="px-16 py-3 w-10 bg-gray-100  text-xs leading-4 font-bold  uppercase ">
            Clean Energy (4)
            </th>
            <th className="px-16 py-3 w-10 bg-gray-100  text-xs leading-4 font-bold  uppercase ">
            Environmental Incidents (5)
            </th>
            <th className="px-16 py-3 w-10 bg-gray-100  text-xs leading-4 font-bold  uppercase ">
            ESG Metrics (5)
            </th>
            <th className="px-16 py-3 w-10 bg-gray-100  text-xs leading-4 font-bold  uppercase ">
            ESG Report (4)
            </th>
            <th className="px-16 py-3 w-10 bg-gray-100  text-xs leading-4 font-bold  uppercase ">
            Sustainability (4)
            </th>
          </tr>
        </thead>
        <tbody>
          {tableData.map((rowData, index) => (
            <tr key={index} className={index % 2 === 0 ? 'bg-gray-50 text-center' : 'bg-white text-center'}>
              <td className="px-2">
                <div className="flex items-center gap-2">
                  <img
                    src={rowData.logoUrl}
                    alt={`${rowData.company} Logo`}
                    className="h-8 w-8 rounded-full mr-2"
                  />
                  {rowData.company}
                </div>
              </td>
              <td className="px-16 py-3 ">{index === 0 ||1 || 2|| 3||4 ||5 ? (
                  <img
                    src={rowData.col1} // Use the SVG URL directly
                    alt="Checkmark"
                    className="h-6 w-6 text-green-500"
                  />
                ) : rowData.col1}</td>
              <td className="px-16 py-3 ">{index === 0 ||1 || 2|| 3||4 ||5 ? (
                  <img
                    src={rowData.col2} // Use the SVG URL directly
                    alt="Checkmark"
                    className="h-6 w-6 text-green-500"
                  />
                ) : rowData.col2}</td>
              <td className="px-16 py-3 ">{index === 0 ||1 || 2|| 3||4 ||5 ? (
                  <img
                    src={rowData.col3} // Use the SVG URL directly
                    alt="Checkmark"
                    className="h-6 w-6 text-green-500"
                  />
                ) : rowData.col3}</td>
              <td className="px-16 py-3 ">
                {index === 0 ||1 || 2|| 3||4 ||5 ? (
                  <img
                    src={rowData.col4} // Use the SVG URL directly
                    alt="Checkmark"
                    className="h-6 w-6 text-green-500"
                  />
                ) : rowData.col4}</td>
              <td className="px-16 py-3 ">{index === 0 ||1 || 2|| 3||4 ||5 ? (
                  <img
                    src={rowData.col5} // Use the SVG URL directly
                    alt="Checkmark"
                    className="h-6 w-6 text-green-500"
                  />
                ) : rowData.col5}</td>
              <td className="px-16 py-3 ">{index === 0 ||1 || 2|| 3||4 ||5 ? (
                  <img
                    src={rowData.col6} // Use the SVG URL directly
                    alt="Checkmark"
                    className="h-6 w-6 text-green-500"
                  />
                ) : rowData.col6}</td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  );
};

export default Table;
